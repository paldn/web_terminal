...
class IndexPageHandler(MixinHandler, tornado.web.RequestHandler):
    def initialize(self, loop):
        super(IndexPageHandler, self).initialize(loop)
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(IndexPageHandler, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(IndexPageHandler, self).check_origin(origin):
...
class SSHPageHandler(MixinHandler, tornado.web.RequestHandler):

    executor = ThreadPoolExecutor(max_workers=cpu_count()*5)

    def initialize(self, loop, policy, host_keys_settings):
        super(SSHPageHandler, self).initialize(loop)
        self.policy = policy
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(SSHPageHandler, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(SSHPageHandler, self).check_origin(origin):
                raise tornado.web.HTTPError(
                    403, 'Cross origin operation is not allowed.'
                )
...
class TelnetPageHandler(MixinHandler, tornado.web.RequestHandler):

    def initialize(self, loop):
        super(TelnetPageHandler, self).initialize(loop)
        self.debug = self.settings.get('debug', False)
        self.font = self.settings.get('font', '')
        self.result = dict(id=None, status=None, encoding=None)
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(TelnetPageHandler, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(TelnetPageHandler, self).check_origin(origin):
                raise tornado.web.HTTPError(
                    403, 'Cross origin operation is not allowed.'
                )
    ...
    def get(self):
        self.render('webtelnet.html', debug=self.debug, font=self.font)


#start

class SSHTerminalHandler(MixinHandler, tornado.web.RequestHandler):

    executor = ThreadPoolExecutor(max_workers=cpu_count()*5)

    def initialize(self, loop, policy, host_keys_settings):
        super(SSHTerminalHandler, self).initialize(loop)
        self.policy = policy
        self.host_keys_settings = host_keys_settings
        self.ssh_client = self.get_ssh_client()
        self.debug = self.settings.get('debug', False)
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(SSHTerminalHandler, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(SSHTerminalHandler, self).check_origin(origin):
                raise tornado.web.HTTPError(
                    403, 'Cross origin operation is not allowed.'
                )
    ...
    def get(self):
        self.render('ssh_terminal.html', debug=self.debug, font=self.font)
...
class TelnetTerminalHandler(MixinHandler, tornado.web.RequestHandler):

    def initialize(self, loop):
        super(TelnetTerminalHandler, self).initialize(loop)
        self.debug = self.settings.get('debug', False)
        self.font = self.settings.get('font', '')
        self.result = dict(id=None, status=None, encoding=None)
    ...
    def write_error(self, status_code, **kwargs):
        if swallow_http_errors and self.request.method == 'POST':
            ...
            super(TelnetTerminalHandler, self).write_error(status_code, **kwargs)
    ...
    def check_origin(self):
        ...
        if origin:
            if not super(TelnetTerminalHandler, self).check_origin(origin):
                raise tornado.web.HTTPError(
                    403, 'Cross origin operation is not allowed.'
                )
    ...
    def get(self):
        self.render('telnet_terminal.html', debug=self.debug, font=self.font)
